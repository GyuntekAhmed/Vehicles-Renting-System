namespace Vehicle2Go.Services.Data
{
    ...
    public class TruckService : ITruckService
    {
        ...
        public async Task<VehiclePreDeleteDetailsViewModel> GetTruckForDeleteByIdAsync(string truckId)
        {
            Truck truck = await this.dbContext
                .Trucks
                .Where(t => t.IsActive)
        ...
        public async Task DeleteByIdAsync(string truckId)
        {
            Truck truckToDelete = await this.dbContext
                .Trucks
                .Where(t => t.IsActive)
                .FirstAsync(t => t.Id.ToString() == truckId);
        ...
        public async Task<bool> IsRentedByIdAsync(string truckId)
        {
            Truck truck = await this.dbContext
                .Trucks
                .FirstAsync(c => c.Id.ToString() == truckId);

            return truck.RenterId.HasValue;
        }

        public async Task RentTruckAsync(string truckId, string userId)
        {
            Truck truck = await this.dbContext
                .Trucks
                .FirstAsync(c => c.Id.ToString() == truckId);

            truck.RenterId = Guid.Parse(userId);

            await this.dbContext.SaveChangesAsync();
        }

        public async Task<bool> IsRentedByUserWithIdAsync(string truckId, string userId)
        {
            Truck truck = await this.dbContext
                .Trucks
                .FirstAsync(c => c.Id.ToString() == truckId);
            ...
            return truck.RenterId.HasValue &&
                   truck.RenterId.ToString() == userId;
        }

        public async Task LeaveAsync(string truckId)
        {
            Truck truck = await this.dbContext
                .Trucks
                .FirstAsync(c => c.Id.ToString() == truckId);
        ...
        public async Task<StatisticsServiceModel> GetStatisticsAsync()
        {
            ...
            {
                TotalVehicles = await this.dbContext.Trucks.CountAsync(),
